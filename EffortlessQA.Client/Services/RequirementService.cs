	...
	public class RequirementService
	{
		...
		)
		{
			...
			catch (Exception ex)
			{
				...
				throw new Exception($"Unexpected error while fetching requirements: {ex.Message}",ex);
			}
		}
		...
		)
		{
			...
			{
				Console.WriteLine($"Unexpected Error: {ex.Message}\nStackTrace: {ex.StackTrace}");
				throw new Exception($"Unexpected error while creating requirement: {ex.Message}",ex);
			}
		}
		...
		)
		{
			...
			{
				Console.WriteLine($"Unexpected Error: {ex.Message}\nStackTrace: {ex.StackTrace}");
				throw new Exception($"Unexpected error while updating requirement: {ex.Message}",ex);
			}
		}

		public async Task DeleteRequirementAsync( Guid id , Guid ProjectId,CancellationToken cancellationToken = default )
		{
			try
			{
				if (id == Guid.Empty)
				...
				var response = await _httpClient.DeleteAsync($"/api/v1/projects/{ProjectId}/requirements/{id}",cancellationToken);
				var responseContent = await response.Content.ReadAsStringAsync(cancellationToken);
				Console.WriteLine(
			...
			{
				Console.WriteLine($"Unexpected Error: {ex.Message}\nStackTrace: {ex.StackTrace}");
				throw new Exception($"Unexpected error while deleting requirement: {ex.Message}",ex);
			}
		}
